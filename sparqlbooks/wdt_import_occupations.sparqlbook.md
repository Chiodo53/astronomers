...
You will be aple to observe that there are multiple parallel classifications in Wikidata, and the fact that of using the same class and parenthood property adds more levels than expected!

You can then transverse them recursively with property paths.
...
# This level is present in the data although not explicitely inserted

# no result after first insert
# result after second insert !
...
WHERE {
    GRAPH <https://github.com/Sciences-historiques-numeriques/astronomers/blob/main/graphs/wikidata-imported-data.md>
        ...
        ## property path:
        ?s  wdt:P106/wdt:P279/wdt:P279/wdt:P279/wdt:P279  ?parentOccupation
...
```
## Add the *instance of* metaclasses to occupations

**CAUTION** : add these classifications ONLY after you are finished with your three (or more) inserts of occupations as parents of other occupations
...
SELECT ?metaclass ?metaclassLabel (COUNT(*) as ?n)
WHERE
    {
        GRAPH <https://github.com/Sciences-historiques-numeriques/astronomers/blob/main/graphs/wikidata-imported-data.md>
        ## Find the persons in the imported graph
        ...
        SERVICE <https://query.wikidata.org/sparql>
            {
                # instance of
                ?item wdt:P31 ?metaclass.
                BIND (?metaclassLabel as ?metaclassLabel)
                SERVICE wikibase:label { bd:serviceParam wikibase:language "en". } 
            }
                
        }
GROUP BY ?metaclass ?metaclassLabel
ORDER BY DESC(?n)
LIMIT 20
...
WHERE
    {
        ...
        SERVICE <https://query.wikidata.org/sparql>
            {
                # instance of, DEUX niveaux
                ?item wdt:P31/wdt:P31 ?metaclass.
                BIND (?metaclassLabel as ?metaclassLabel)
                SERVICE wikibase:label { bd:serviceParam wikibase:language "en". } 
            }
...
```sparql
## Insert metaclasses, first level
...
SELECT ?personLabel ?birthYear ?item
...
CONSTRUCT {
          ?parentOccupation  wdt:P425 ?occupationField.
          ?occupationField rdfs:label ?occupationFieldLabel.
          ?occupationField wdt:P31 ?knowledgeClassification.
          ?knowledgeClassification rdfs:label ?knowledgeClassificationLabel.
          }
WHERE {
    GRAPH <https://github.com/Sciences-historiques-numeriques/astronomers/blob/main/graphs/wikidata-imported-data.md>
        {
          SELECT DISTINCT ?parentOccupation ?parentOccupationlabel 
...
INSERT DATA {
  GRAPH <https://github.com/Sciences-historiques-numeriques/astronomers/blob/main/graphs/wikidata-imported-data.md>
  {wdt:P425 rdfs:label 'field of this occupation'.}
}
```
...
SELECT ?parentOccupationlabel ?occupationFieldLabel ?knowledgeClassificationLabel (count(*) as ?n)
WHERE {
    GRAPH <https://github.com/Sciences-historiques-numeriques/astronomers/blob/main/graphs/wikidata-imported-data.md>
        ...
        {?s a wd:Q5.  
          ...
          OPTIONAL {?parentOccupation rdfs:label ?parentOccupationlabel}    
          OPTIONAL {?occupationField rdfs:label ?occupationFieldLabel}    
          OPTIONAL {?knowledgeClassification rdfs:label ?knowledgeClassificationLabel}    
        }
}
GROUP BY ?parentOccupationlabel ?occupationFieldLabel ?knowledgeClassificationLabel
ORDER BY DESC(?n)
limit 20
```
...
SELECT ?knowledgeClassification ?knowledgeClassificationLabel (count(*) as ?n)
WHERE {
    GRAPH <https://github.com/Sciences-historiques-numeriques/astronomers/blob/main/graphs/wikidata-imported-data.md>
...
GROUP BY ?knowledgeClassification ?knowledgeClassificationLabel
ORDER BY DESC(?n)    
```
